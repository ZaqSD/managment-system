...
export default function CreateContentCustomer(props: CreateContentCustomerProps) {
    ...
    const [updateCustomer, setUpdateCustomer] = React.useState<customerProps>();
    const [updateAddresses, setUpdateAddresses] = React.useState<addressProps[]>([]);

    function GetCustomer(id: string | undefined){
        const [customer, setCustomer] = React.useState<customerProps>();
    
        fetch('http://localhost:8080/customer/' + id, {method: 'GET'})
            .then((response) => response.json())
            .then((data) => {
                console.log(data);
                setCustomer(data);
    ...
    const UseFetchData = () => {
        React.useEffect(() => {
            if (props.update === true) {
                setUpdateCustomer(GetCustomer(props.customerId));
                setUpdateAddresses(GetAddressesByCustomerId(props.customerId));
            }
        }, []);
    ...
    return (
        <>
            <TextField 
                ...
                variant="outlined" 
                sx={{marginTop: 2}} 
                defaultValue={updateCustomer?.name}
